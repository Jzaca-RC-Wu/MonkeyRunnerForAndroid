# -*- coding: utf-8 -*-
from com.android.monkeyrunnner import  MonkeyDevice, MonkeyRunner,MonkeyImage
...
def getdevice(timeout=None,serialno_num =None):
    '''
    ...
    :return: 
    '''
    device = MonkeyRunner.waitForConnection(timeout,serialno_num)
    return device

#get devise display attributes
def get_device_display(device):
    '''
...
def get_current_package(device):
    return device.getProperty("am.cuurent.package")

def get_current_action(deivce):
    return deivce.getProperty('am.current.action')

#获取当前运行的类
def get_current_Class(device):
    return device.getProperty('am.current.comp.class')

#获取某组件所在的包名
def get_comp_package(device,comp):
    return device.getProperty("am.current.comp.package")

#获取当前Activity的数据
def get_current_ActivityData(device):
    return device.getProperty('am.current.data')

#获取当前Activity的Categories
def get_current_ActivityCate(device):
    return device.getProperty('am.current.categories')
